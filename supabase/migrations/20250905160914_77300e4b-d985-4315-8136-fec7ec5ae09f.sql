...
-- 4) Seed a few non-featured published articles (check if they exist first)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.articles WHERE title = 'Designing a URL Shortener like Bitly') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.articles WHERE title = 'Building a Rate Limiter Service') THEN
        INSERT INTO public.articles (title, content, excerpt, company, read_time, tags, difficulty, published, is_featured)
        VALUES (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.articles WHERE title = 'How a News Feed Works (Beginner Guide)') THEN
        INSERT INTO public.articles (title, content, excerpt, company, read_time, tags, difficulty, published, is_featured)
        VALUES (
    ...
    IF NOT EXISTS (SELECT 1 FROM public.articles WHERE title = 'Job Queue and Worker System Basics') THEN
        INSERT INTO public.articles (title, content, excerpt, company, read_time, tags, difficulty, published, is_featured)
        VALUES (
